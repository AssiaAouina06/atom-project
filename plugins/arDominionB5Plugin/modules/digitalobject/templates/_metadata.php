            ...
            <div class="accordion-body p-0">
              <?php if ($showOriginalFileMetadata) { ?>
                ...
                <div class="<?php echo render_b5_show_field_css_classes(); ?>">
                  ...
                  <h3 class="<?php echo render_b5_show_label_css_classes(); ?>"><?php echo __('Original file'); ?>
                  <i class="fa fa-archive ms-2 text-dark<?php if (!$canAccessOriginalFile) { ?> text-muted<?php } ?>" aria-hidden="true"></i></h3>

                  <div class="digital-object-metadata-body <?php echo render_b5_show_value_css_classes(); ?>">
                    <?php if ($showOriginalFileName) { ?>
                      <?php echo render_show(__('Filename'), render_value_inline($resource->object->originalFileName), 
                      ['fieldLabel' => 'originalFileName', 'isSubField' => true]); ?>
                    <?php } ?>

                    <?php if ($showOriginalFormatName) { ?>
                      <?php echo render_show(__('Format name'), render_value_inline($resource->object->formatName), 
                      ['fieldLabel' => 'originalFileFormatName', 'isSubField' => true]); ?>
                    <?php } ?>

                    <?php if ($showOriginalFormatVersion) { ?>
                      <?php echo render_show(__('Format version'), render_value_inline($resource->object->formatVersion), 
                      ['fieldLabel' => 'originalFileFormatVersion', 'isSubField' => true]); ?>
                    <?php } ?>

                    <?php if ($showOriginalFormatRegistryKey) { ?>
                      <?php echo render_show(__('Format registry key'), render_value_inline($resource->object->formatRegistryKey),
                       ['fieldLabel' => 'originalFileFormatRegistryKey', 'isSubField' => true]); ?>
                    <?php } ?>

                    <?php if ($showOriginalFormatRegistryName) { ?>
                      <?php echo render_show(__('Format registry name'), render_value_inline($resource->object->formatRegistryName),
                       ['fieldLabel' => 'originalFileFormatRegistryName', 'isSubField' => true]); ?>
                    <?php } ?>

                    <?php if ($showOriginalFileSize) { ?>
                      <?php echo render_show(__('Filesize'), hr_filesize(intval((string) $resource->object->originalFileSize)),
                       ['fieldLabel' => 'originalFileSize', 'isSubField' => true]); ?>
                    <?php } ?>
                  ...
                  <div class="digital-object-metadata-body <?php echo render_b5_show_value_css_classes(); ?>">
                    ...
                    <?php if ($showMasterFileName) { ?>
                      <?php if ($canAccessMasterFile) { ?>
                        <?php echo render_show(__('Filename'), link_to($resource->name, $resource->object->getDigitalObjectUrl(),
                         ['target' => '_blank']), ['fieldLabel' => 'filename', 'isSubField' => true]); ?>
                      <?php } else { ?>
